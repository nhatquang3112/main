...
The To-do list feature is facilitated by `UniqueToDoList`, which resides inside `AddressBook`. It supports storing a list of to-do inside the address book.
Each to-do has a content, showing the activities of the to-do and a status, indicating if the to-do is done or undone.
A to-do, content and status are facilitated by `ToDo`, `Content` and `Status` respectively.

The following figure shows the class diagram indicating the relationship between `ToDo`, `Content` and `Status`:

image::AddressBookUniqueToDoListToDoClassDiagram.png[width="800"]
Figure. Class diagram of AddressBook, UniqueToDoList, ToDo, Content and Status

On a broader scale, the following figure shows the class diagram of `AddressBook`, `UniqueToDoList`, `ToDo`, `Content` and `Status`:
